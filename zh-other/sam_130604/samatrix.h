...
class symmetric_associative_matrix
{
...
public:
    ...
    symmetric_associative_matrix():N(0){}
    ...
    reference operator()(const Index& r , const Index& c)
    {
        ...
        if ( ( ( rit = indexes.find(r) ) != indexes.end() ) && ( ( cit = indexes.find(c) ) != indexes.end() ) )
        {
            ...
            // ha az alsóháromszögbe vannak, kicseréljük a koordinátákat, majd lekérjük a mátrix elemét
            if( rnum < cnum )
            {
                std::swap( rnum , cnum );
            }
    ...
    bool operator==( const symmetric_associative_matrix& tr ) const
    {
    ...
    // összeadás operátor
    symmetric_associative_matrix operator+( const symmetric_associative_matrix& tr) const
    {
        // hozzuk létre az eredményünket a meghívott mátrix alapján
        symmetric_associative_matrix tmp = *this;
        ...
        {
            for ( first2 = first1 ; first2 != last1 ; ++first2 )
            {
